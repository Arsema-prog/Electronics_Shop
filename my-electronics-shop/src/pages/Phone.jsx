...
const Phone = ({ onProductSelect }) => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:3000/api_product_details.php') // Ensure this URL is correct
            .then((res) => res.json())
            .then((data) => {
                // Filter for products in the "phone" category
                const phoneProducts = data.filter(product => product.category.toLowerCase() === 'phone');
                setProducts(phoneProducts);
                setDisplayedProducts(phoneProducts);
            })
