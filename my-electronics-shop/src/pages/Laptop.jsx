...
const Laptop = ({ onProductSelect }) => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:3000/api_product_details.php') // Ensure this URL is correct
            .then((res) => res.json())
            .then((data) => {
                // Filter for products in the "laptop" category
                const laptopProducts = data.filter(product => product.category.toLowerCase() === 'laptop');
                setProducts(laptopProducts);
                setDisplayedProducts(laptopProducts);
            })
