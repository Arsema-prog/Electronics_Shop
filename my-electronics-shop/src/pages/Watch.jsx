...
const Watch = ({ onProductSelect }) => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:3000/api_product_details.php') // Ensure this URL is correct
            .then((res) => res.json())
            .then((data) => {
                // Filter for products in the "watch" category
                const watchProducts = data.filter(product => product.category.toLowerCase() === 'watch');
                setProducts(watchProducts);
                setDisplayedProducts(watchProducts);
            })
