...
const Navbar = ({ onCategoryChange, products, onProductSelect }) => {
  ...
  return (
    <nav className="flex flex-col md:flex-row justify-between items-center p-4 bg-[#CBD0D8] text-white fixed top-0 left-0 right-0 z-50">
      <div className="text-xl md:text-2xl lg:text-3xl text-black font-bold">
      ...
      >
        {['Laptop', 'Phone', 'Watch', 'Perfume'].map((category) => (
          <li key={category} className="cursor-pointer text-black hover:text-gray-400 text-lg md:text-base lg:text-lg py-2 md:py-0 md:px-2">
          <Link to={`/${category.toLowerCase()}`} onClick={() => setIsMenuOpen(false)}>
            {category}
          </Link>
        </li>
      ))}
